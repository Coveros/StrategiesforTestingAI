...
1. Gate decision status should drive recommendation order before subjective preference.
2. Pass-rate drop and latency warnings must be interpreted together, not in isolation.
3. Candidate drift can invalidate release readiness even when core safety checks pass.

### Likely Issues, Defects, or Quality Challenges
1. Teams may overrule FAIL outcomes without explicit mitigation and rollback criteria.
...
3. Add one follow-up automation check for any warning reason before final Ship approval.
